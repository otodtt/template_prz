...
@section('content')
    <div class="container">
        <div class="row">
            <div class="row">
                <a href="{!!URL::to('/nematocides/'.$limatside->id)!!}"><- {{$limatside->name}}</a>
            </div>
            <div class="col-lg-12">
                @if(count($errors)>0)
                ...
                @endif
                {!! Form::model($subs, ['url'=>'limatsides/subs_update/'.$subs[0]['id'].'/'.$subs[0]['pesticides_id'] , 'method'=>'POST', 'id'=>'form', 'class'=>'form-horizontal']) !!}
                <fieldset>
                    <legend style="color: blue">РЕДАКТИРАНЕ НА АКТИВНО ВЕЩЕСТВО КЪМ {{$limatside->name}}</legend>
