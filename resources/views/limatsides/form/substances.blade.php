...
@section('content')
    <div class="container">
        <div class="row">
            <div class="row">
                <a href="{!!URL::to('/limatsides/'.$limatsides->id)!!}"><- {{$limatsides->name}}</a>
            </div>
            <div class="col-lg-12">
                @if(count($errors)>0)
                ...
                @endif
                {!! Form::open(['url'=>'limatsides/subs_add/'.$limatsides->id , 'method'=>'POST', 'id'=>'form', 'class'=>'form-horizontal']) !!}
                <fieldset>
                    <legend style="color: blue">ДОБАВЯНЕ НА АКТИВНО ВЕЩЕСТВО КЪМ {{$limatsides->name}}</legend>
