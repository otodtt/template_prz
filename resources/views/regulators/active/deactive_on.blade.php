...
@section('content')
    <div class="container">
        ...
        <div class="row" style="text-align: center; color: green">
            <h4>АКТИВИРАНЕ ИЛИ ДЕАКТИВИРАНЕ НА ДОЗА ОТ ПРЗ {{$regulator->name}}</h4>
        </div>
        <div class="col-lg-12">
            @if(count($errors)>0)
        ...
        <div class="col-lg-12">
            ...
            <p > {{$regulator->name}} - {{ $doses[0]['dose'] }} {{ $doses[0]['measure'] }}</p>
            <p >{!! $doses[0]['disease'] !!}</p>
        </div>
        <div class="col-lg-12" style="margin: 0 auto">
            {!! Form::model($doses[0], ['url'=>'regulators/deactivate_one_store/'.$doses[0]['id'].'/'.$regulator->id , 'method'=>'POST', 'id'=>'form', 'class'=>'form-horizontal']) !!}
